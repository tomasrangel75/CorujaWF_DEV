    ...
    public partial class FormAdm : Form
    {
        ...
        private void btnIniAval_Click(object sender, EventArgs e)
        {
            ...
            if (rdNewAval.Checked == true)
            {
                ...
                aval.Prova_id = idPr;
                aval.CheckStatus();
                string res1 = aval.Status;

                if (!String.IsNullOrEmpty(res1))
                {
                    if (res1.Equals("Iniciada"))
                    {
            ...
            // int res = CheckForCrashed(prova, projPath, idTipoProva);
            ...
            ConfigurationManager.AppSettings.Set("novaAval", isNew.ToString()); // X
            ConfigurationManager.AppSettings.Set("nomeProva", prova);
            ConfigurationManager.AppSettings.Set("idAval", idAval); //X
            ConfigurationManager.AppSettings.Set("idTipoProva", idTipoProva); //X
            ConfigurationManager.AppSettings.Set("esp", esp); //X
            ConfigurationManager.AppSettings.Set("pac", pac); //X
            ConfigurationManager.AppSettings.Set("idPac", idPac.ToString()); //X
            ConfigurationManager.AppSettings.Set("dt", dtNasc); //X
            ConfigurationManager.AppSettings.Set("sexo", sexo); //X
                                                                // ConfigurationManager.AppSettings.Set("isUpdt", res.ToString() ); //X
        ...
        private static int CheckForCrashed(string prova, string projPath, string idTipoProva)
        {
            int result = 0;

            string fullPath = projPath + "\\" + prova + "\\DBQUEST";
            if (File.Exists(fullPath))
            {

                string connCheck = ConfigurationManager.
                     ConnectionStrings["Banco"].ConnectionString;

                connCheck = connCheck.Replace("#PARAMETRO#", fullPath);

                using (QuestEntities qEntities = new QuestEntities(connCheck))
                {
                    var esp = qEntities.Turma.FirstOrDefault();
                    var pac = qEntities.Aluno.FirstOrDefault();
                    var q = qEntities.Questionario.FirstOrDefault();


                    if (pac != null && esp != null)
                    {
                        var pacCE = new Paciente();
                        pacCE.Nome = pac.Nome;
                        string idPac_ = pacCE.GetIdPac(esp.Nome).ToString();

                        var pacCrashedFileName = idPac_ + pac.Nome + idTipoProva;

                        //copy db to temp with ids 
                        File.Copy(fullPath, projPath + "\\" + prova + "\\DbTemp" + "\\" + pacCrashedFileName, true);
                        //delete db in the main root + //replace with db from storage
                        File.Copy(projPath + "\\" + prova + "\\DbStorage" + "\\DBQUEST", projPath + "\\" + prova + "\\DBQUEST", true);
                        result = 1;
